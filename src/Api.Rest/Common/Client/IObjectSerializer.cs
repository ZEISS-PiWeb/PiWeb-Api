...
namespace Zeiss.PiWeb.Api.Rest.Common.Client
{
	...
	public interface IObjectSerializer
	{
		...
		Task SerializeAsync<T>( [NotNull] Stream stream, T value, CancellationToken cancellationToken );
		...
		Task<T> DeserializeAsync<T>( [NotNull] Stream stream, CancellationToken cancellationToken );
		...
		/// <param name="cancellationToken">The <see cref="CancellationToken"/> that can be used to cancel the operation.</param>
		/// <exception cref="ObjectSerializerException">Error during deserialization.</exception>
		IAsyncEnumerable<T> DeserializeAsyncEnumerable<T>( [NotNull] Stream stream, CancellationToken cancellationToken );
	}
}